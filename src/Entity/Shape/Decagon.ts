...
export default class Decagon extends AbstractShape {
    ...
    public constructor(game: GameServer, isAlpha=false, alpha=(Math.random() < 0.035), shiny=(Math.random() < 0.01) && !isAlpha) {
        ...
        this.nameData.values.name = isAlpha ? "Alpha Decagon" : "Decagon";

        this.healthData.values.health = this.healthData.values.maxHealth = (isAlpha ? 64000 : 6000);
        this.physicsData.values.size = (isAlpha ? 300 : 140) * Math.SQRT1_2;
        this.physicsData.values.sides = 10;
        this.styleData.values.color = shiny ? Color.Shiny : Color.EnemyDecagon;

        this.physicsData.values.absorbtionFactor = isAlpha ? 0.005 : 0.01;
        ...
        this.scoreReward = isAlpha ? 50000 : 6000;
