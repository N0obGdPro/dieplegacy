...
export default class Nonagon extends AbstractShape {
    ...
    public constructor(game: GameServer, isAlpha=false, alpha=(Math.random() < 0.035), shiny=(Math.random() < 0.01) && !isAlpha) {
        ...
        this.nameData.values.name = isAlpha ? "Alpha Nonagon" : "Nonagon";

        this.healthData.values.health = this.healthData.values.maxHealth = (isAlpha ? 32000 : 4500);
        this.physicsData.values.size = (isAlpha ? 250 : 130) * Math.SQRT1_2;
        this.physicsData.values.sides = 9;
        this.styleData.values.color = shiny ? Color.Shiny : Color.EnemyNonagon;

        this.physicsData.values.absorbtionFactor = isAlpha ? 0.01 : 0.02;
        this.physicsData.values.pushFactor = 11;

        this.isAlpha = isAlpha;
        this.isShiny = shiny;

        this.damagePerTick = isAlpha ? 35 : 25;
        this.scoreReward = isAlpha ? 30000 : 4800;
