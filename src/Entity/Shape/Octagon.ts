...
export default class Octagon extends AbstractShape {
    ...
    public constructor(game: GameServer, isAlpha=false, alpha=(Math.random() < 0.035), shiny=(Math.random() < 0.01) && !isAlpha) {
        ...
        this.nameData.values.name = isAlpha ? "Alpha Octagon" : "Octagon";

        this.healthData.values.health = this.healthData.values.maxHealth = (isAlpha ? 25000 : 3200);
        this.physicsData.values.size = (isAlpha ? 225 : 120) * Math.SQRT1_2;
        this.physicsData.values.sides = 8;
        this.styleData.values.color = shiny ? Color.Shiny : Color.EnemyOctagon;

        this.physicsData.values.absorbtionFactor = isAlpha ? 0.01 : 0.05;
        this.physicsData.values.pushFactor = 11;

        this.isAlpha = isAlpha;
        this.isShiny = shiny;

        this.damagePerTick = isAlpha ? 25 : 20;
        this.scoreReward = isAlpha ? 22000 : 3600;
